...
Streamlit UI (`streamlit run app/main.py`): загрузка PDF/DOCX/XLSX «До» и «После»,
демо на тестовом комплекте, пять вкладок результатов, фильтры и подтверждение/отклонение находок.
Анализ пока заменён локальной заглушкой: документы не анализируются, показаны только
учебные примеры с непроверенными цитатами. Вызовы API не выполняются.
